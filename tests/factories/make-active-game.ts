...
export function makeActiveGame(options: MakeActiveGameOptions = {}): CurrentGame {
  ...
  const bannedChampionsCount =
    options.bannedChampionsCount || faker.number.int({ min: 0, max: 10 });

  return {
    gameId: options.gameId || faker.number.int({ min: 1000000000, max: 9999999999 }),
    gameType:
      options.gameType ||
      faker.helpers.arrayElement(['CUSTOM_GAME', 'MATCHED_GAME', 'TUTORIAL_GAME']),
    ...
    gameMode:
      options.gameMode ||
      faker.helpers.arrayElement(['CLASSIC', 'ARAM', 'URF', 'NEXUS_BLITZ', 'ULTBOOK']),
    ...
    gameQueueConfigId:
      options.gameQueueConfigId ||
      faker.helpers.arrayElement([0, 400, 420, 430, 440, 450, 700, 900, 1020, 1300]),
    observers: {
    ...
    participants: Array.from({ length: participantsCount }, (_, index) => ({
      ...
      perks: {
        perkIds: Array.from({ length: faker.number.int({ min: 6, max: 9 }) }, () =>
          faker.number.int({ min: 8000, max: 9000 }),
        ),
...
export function makeActiveGameArray(
  count: number,
  options: MakeActiveGameOptions = {},
): CurrentGame[] {
